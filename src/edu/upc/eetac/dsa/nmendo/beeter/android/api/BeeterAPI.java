...
import edu.upc.eetac.dsa.nmendo.beeter.android.MediaType;
...
public class BeeterAPI {
	...
	private void getRootAPI() throws BeeterAndroidException {
		...
		try {
			...
			// lo pasa a objeto JSON para poder trabajar con él y obtener,
			// ejemplo, links.
			// es decir, obtiene el valor del atributo del objeto llamada
			// 'links'.
			JSONObject jsonObject = new JSONObject(sb.toString());
			// se le pasa el array de links y el mapa del model rootAPI.getLinks
	...
	public StingCollection getStings() throws BeeterAndroidException {
		...
		try {
			...
			// aquí se leen los enlaces y se obtienen los valores de los
			// atributos/parámetros links,
			// newestTimestampo y oldes, stings..
			...
			// se sacan para cada sting los parametros de author, id,
			// lastmodified...
	...
	// en este método es donde se porcesan los links
	// Mapa por el conjunto de parámetros
	private void parseLinks(JSONArray jsonLinks, Map<String, Link> map)
		...
		for (int i = 0; i < jsonLinks.length(); i++) {
			...
			String rels[] = rel.split("\\s"); // para trocear y quitar los
												// espacios en blanco
			for (String s : rels)
				map.put(s, link);
		}
